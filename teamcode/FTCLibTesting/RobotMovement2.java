...
import org.firstinspires.ftc.teamcode.Teamcode.teamcode.FTCLibTesting.Drivetrain;
import java.util.ArrayList;

import static org.firstinspires.ftc.teamcode.Teamcode.teamcode.FTCLibTesting.Drivetrain.worldAngle_rad;
import static org.firstinspires.ftc.teamcode.Teamcode.teamcode.FTCLibTesting.Drivetrain.worldXPosition;
import static org.firstinspires.ftc.teamcode.Teamcode.teamcode.FTCLibTesting.Drivetrain.worldYPosition;
...
public class RobotMovement2 {
    ...
    public static void goToPosition(double targetX, double targetY, double point_angle, double movement_speed, double point_speed) {
        ...
        drive.movement_turn = turnPower;
        drive.movement_x = movement_x_power;
        drive.movement_y = movement_y_power;
    ...
    public static movementResult gunToPosition(double targetX, double targetY,double point_angle,
        ...
        drive.movement_x = Range.clip(movement_x_power,-movement_speed,movement_speed);
        drive.movement_y = Range.clip(movement_y_power,-movement_speed,movement_speed);
        ...
        drive.movement_turn = Range.clip(turnSpeed,-point_speed,point_speed);
        //HOWEVER don't go frantic when right next to the point
        if(distanceToPoint < 10){
            drive.movement_turn = 0;
        }
        ...
        drive.movement_x *= Range.clip((relative_abs_x/6.0),0,1);
        drive.movement_y *= Range.clip((relative_abs_y/6.0),0,1);

        drive.movement_turn *= Range.clip(Math.abs(relativePointAngle)/Math.toRadians(2),0,1);
        ...
        if(Math.abs(drive.movement_turn) < 0.00001){
            errorTurnSoScaleDownMovement = 1;
        }
        drive.movement_x *= errorTurnSoScaleDownMovement;
        drive.movement_y *= errorTurnSoScaleDownMovement;
    ...
    public static movementResult pointAngle(double point_angle, double point_speed, double decelerationRadians) {
        ...
        drive.movement_turn *= Range.clip(Math.abs(relativePointAngle)/Math.toRadians(3),0,1);
    ...
    private static void allComponentsMinPower() {
        if(Math.abs(drive.movement_x) > Math.abs(drive.movement_y)){
            if(Math.abs(drive.movement_x) > Math.abs(drive.movement_turn)){
                drive.movement_x = minPower(drive.movement_x,movement_x_min);
            }else{
                drive.movement_turn = minPower(drive.movement_turn,movement_turn_min);
            }
        }else{
            if(Math.abs(drive.movement_y) > Math.abs(drive.movement_turn)){
                drive.movement_y = minPower(drive.movement_y, movement_y_min);
            }else{
                drive.movement_turn = minPower(drive.movement_turn,movement_turn_min);
            }
        }
    ...
    public static boolean followCurve(ArrayList<CurvePoint> allPoints, double followAngle, boolean allowSkipping){
        ...
        drive.movement_x *= 1 - Range.clip(Math.abs(result.turnDelta_rad) / followMe.slowDownTurnRadians,0,followMe.slowDownTurnAmount);
        drive.movement_y *= 1 - Range.clip(Math.abs(result.turnDelta_rad) / followMe.slowDownTurnRadians,0,followMe.slowDownTurnAmount);
    ...
    public static myPoint pointAlongLine(double lineX1, double lineY1, double lineX2, double lineY2,
        ...
        //FIGURE OUT IF WE ARE ACTUALLY ON THE LINE SEGMENT
