...
public class RobotMovement2 {
    ...
    public static void goToPosition(double targetX, double targetY, double point_angle, double movement_speed, double point_speed) {
        ...
        Robot.movement_turn = turnPower;
        Robot.movement_x = movement_x_power;
        Robot.movement_y = movement_y_power;
    ...
    public static movementResult gunToPosition(double targetX, double targetY,double point_angle,
        ...
        Robot.movement_x = Range.clip(movement_x_power,-movement_speed,movement_speed);
        Robot.movement_y = Range.clip(movement_y_power,-movement_speed,movement_speed);
        ...
        Robot.movement_turn = Range.clip(turnSpeed,-point_speed,point_speed);
        ...
        Robot.movement_x *= Range.clip((relative_abs_x/6.0),0,1);
        Robot.movement_y *= Range.clip((relative_abs_y/6.0),0,1);

        Robot.movement_turn *= Range.clip(Math.abs(relativePointAngle)/Math.toRadians(2),0,1);
        ...
        if(Math.abs(Robot.movement_turn) < 0.00001){
            errorTurnSoScaleDownMovement = 1;
        }
        Robot.movement_x *= errorTurnSoScaleDownMovement;
        Robot.movement_y *= errorTurnSoScaleDownMovement;
    ...
    public static movementResult pointAngle(double point_angle, double point_speed, double decelerationRadians) {
        ...
        Robot.movement_turn *= Range.clip(Math.abs(relativePointAngle)/Math.toRadians(3),0,1);
    ...
    private static void allComponentsMinPower() {
        if(Math.abs(Robot.movement_x) > Math.abs(Robot.movement_y)){
            if(Math.abs(Robot.movement_x) > Math.abs(Robot.movement_turn)){
                Robot.movement_x = minPower(Robot.movement_x,movement_x_min);
            }else{
                Robot.movement_turn = minPower(Robot.movement_turn,movement_turn_min);
            }
        }else{
            if(Math.abs(Robot.movement_y) > Math.abs(Robot.movement_turn)){
                Robot.movement_y = minPower(Robot.movement_y, movement_y_min);
            }else{
                Robot.movement_turn = minPower(Robot.movement_turn,movement_turn_min);
            }
        }
    ...
    public static boolean followCurve(ArrayList<CurvePoint> allPoints, double followAngle, boolean allowSkipping){
        ...
        Robot.movement_x *= 1 - Range.clip(Math.abs(result.turnDelta_rad) / followMe.slowDownTurnRadians,0,followMe.slowDownTurnAmount);
        Robot.movement_y *= 1 - Range.clip(Math.abs(result.turnDelta_rad) / followMe.slowDownTurnRadians,0,followMe.slowDownTurnAmount);
